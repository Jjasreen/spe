...
@section('content')
<h1>Create Polling Question for {{$polling->polling_title}}</h1>
{{Form::open()}}
{{Form::token()}}
...
<div>
    ...
    {{Form::text('polling_question_number','', ['class'=>'form-control'])}}
</div>

<div>
    {{Form::label("Question", null, ['class'=>'form-label'])}}
    {{Form::text('polling_question','', ['class'=>'form-control'])}}
</div>
